...
class correNuevo{
  ...
  analisis(){
    setTimeout(()=>{
    if(document.getElementsByClassName("speedModePanel")[0].innerText){
  ...
  inicializarBusqueda(){
   ...
   var CVP = setInterval(CVPSEGMENTO,3000);
        ...
        if(line_x===array[numeroMAS]){
         ...
         // return this.Pulsar_Enter_QD();
          ...
          line_DE_COMANDOS[0].value="QD"
           ...
           return  setTimeout(()=> new correNuevo(),1700)
    ...
    localStorage.setItem('CVP_NUMERO',array[numeroMAS]);
        ...
        var line_DE_COMANDOS=document.getElementsByClassName("cmdPromptInput");
            ...
            line_DE_COMANDOS[0].dispatchEvent(PRESS_ENTER)
            setTimeout(()=> new correNuevo(),1700);
  ...
  Pulsar_Enter_RT(){
    ...
    line_DE_COMANDOS[0].dispatchEvent(PRESS_ENTER)
    setTimeout(()=> this.inicializarBusqueda(),2500);
